const workData = [
  {
    title: "Recruitment Management System",
    slug: "recruitment-management-system",
    type: "software",
    year: 2024,
    href: "https://employer.kumarijob.com",
    source: null,
    image: "/img/works/employer.jpg",
    company: "Kumari Job",
    description:
      "A sophisticated enterprise-grade recruitment platform enabling seamless job posting, applicant tracking, and hiring workflows for thousands of employers.",
    role: "Fullstack Development",
    stacks: ["Laravel", "React", "Next.js"],
    is_featured: true,
  },
  {
    title: "Training Management System",
    slug: "training-management-system",
    type: "software",
    year: 2024,
    href: "https://tms.kumarijob.com",
    source: null,
    image: "/img/works/tms.jpg",
    company: "Kumari Job",
    description:
      "An end-to-end Learning Management System (LMS) designed to power professional training, featuring online examinations and automated certification tracking.",
    role: "Team Lead",
    stacks: ["Laravel", "React", "Next.js"],
    is_featured: true,
  },
  {
    title: "HRMS",
    slug: "hrms",
    type: "software",
    year: 2024,
    href: "https://hrms.kumarijob.com",
    source: null,
    image: "/img/works/hrms.jpg",
    company: "Kumari Job",
    description:
      "Streamlining human resource operations with a modern module for employee records, asset management, and payroll processing in a unified interface.",
    role: "Team Lead",
    stacks: ["Laravel", "React", "Next.js"],
    is_featured: true,
  },
  {
    title: "Jobseeker App",
    slug: "jobseeker-app",
    type: "mobile",
    year: 2024,
    href: "https://play.google.com/store/search?q=kumari%20job&c=apps&hl=en",
    source: null,
    image: "/img/works/jobseeker_app.jpg",
    company: "Kumari Job",
    description:
      "High-performance mobile application architected for job discovery and rapid application submission, serving a massive user base in Nepal.",
    role: "Backend Development",
    stacks: ["Laravel", "Rest API"],
    is_featured: true,
  },
  {
    title: "Kumari Job Portal",
    slug: "kumari-job-web",
    type: "software",
    year: 2023,
    href: "https://www.kumarijob.com",
    source: null,
    image: "/img/works/kumari_job.png",
    company: "Kumari Job",
    description:
      "Scaling Nepal's leading job portal through robust backend architecture and real-time data synchronization with Firebase.",
    role: "Backend Development",
    stacks: ["Laravel", "Rest API", "Firebase"],
    is_featured: true,
  },
  {
    title: "Pasa HRMS",
    slug: "pasa-hrms",
    type: "software",
    year: 2022,
    href: null,
    source: null,
    image: "/img/works/pasa_hrms.png",
    company: "Pasa International",
    description:
      "Custom-built manpower recruitment software managing large-scale candidate documentation and visa processing workflows for international placement.",
    role: "FullStack Development",
    stacks: ["Laravel", "Docker", "VPS"],
    is_featured: true,
  },
  {
    title: "Pasa International",
    slug: "pasa-international",
    type: "website",
    year: 2022,
    href: "https://pasainternational.com.np",
    source: null,
    image: "/img/works/pasa_website.png",
    company: "Pasa International",
    description:
      "A high-conversion corporate web presence for a leading manpower agency, optimized for international recruitment visibility and authority.",
    role: "FullStack Development",
    stacks: ["Laravel", "Bootstrap"],
    is_featured: true,
  },
  {
    title: "SAS Education Consultancy",
    slug: "sas-education-consultancy",
    type: "website",
    year: 2023,
    href: "https://saseducationconsultancy.com",
    source: null,
    image: "/img/works/sas.png",
    company: "SAS Education",
    description:
      "A distinguished consultancy platform offering exceptional IELTS resources and study abroad guidance for Canada, Australia, UK, and USA.",
    role: "FullStack Development",
    stacks: ["Laravel", "Inertia.js", "React"],
    is_featured: false,
  },
  {
    title: "Asian Bartending and Barista School",
    slug: "asian-bartending-and-barista-achool",
    type: "website",
    year: 2023,
    href: "https://asianbartendingandbaristaschool.com",
    source: null,
    image: "/img/works/asian.jpg",
    company: "Asian Bartending",
    description:
      "An immersive digital platform for a premier hospitality training institute, empowering individuals to thrive in the global beverage industry.",
    role: "FullStack Development",
    stacks: ["Django", "Tailwind"],
    is_featured: false,
  },
  {
    title: "Pace Infosys",
    slug: "pace-infosys",
    type: "website",
    year: 2024,
    href: "https://thepaceinfosys.com",
    source: null,
    image: "/img/works/paceinfosys.jpg",
    description:
      "A beacon of innovation and excellence in digital marketing and software development, showcasing modern design and performance.",
    role: "Project Management",
    stacks: ["Laravel", "React", "Next.js"],
    is_featured: true,
  },
  {
    title: "Al-Rimal Company Website",
    slug: "al-rimal-website",
    type: "website",
    year: 2023,
    href: "https://www.al-rimalgroup.com",
    source: null,
    image: "/img/works/al-rimal.png",
    description:
      "Al-rimal Website is developed for the purpose of blogging to assist user with career tips and self growth blogs. Its website also list various job positions available in UAE",
    role: "Backend Development",
    stacks: ["Django", "DRF", "Next.js", "Tailwind", "SEO"],
    is_featured: false,
  },
  {
    title: "Good Neighbours",
    slug: "good-neighbours-nepal",
    type: "website",
    year: 2023,
    href: "https://goodneighboursnepal.org",
    source: null,
    image: "/img/works/good.png",
    description:
      "Good Neighbors Nepal (GNN) is a non-government organization, established in 2002 at Kathmandu",
    role: "Backend Development",
    stacks: ["Django", "Bootstrap"],
    is_featured: false,
  },
  {
    title: "Developer Portfolio with Next.js",
    slug: "developer-portfolio",
    type: "open source",
    year: 2023,
    href: "https://sarojpradhan.com.np",
    source: null,
    image: "/img/works/portfolio.png",
    description:
      "A Developer portfolio template built in Next.js with Typescript and Tailwind css.",
    role: "Personal Project",
    stacks: ["React", "Next.js", "Tailwind"],
    is_featured: false,
  },
  {
    title: "Gyannhub",
    slug: "gyannhub",
    type: "website",
    year: 2022,
    href: "https://gyannhub.pradhansaroj.com.np",
    source: null,
    image: "/img/works/gyannhub.jpg",
    description:
      "Gyann Hub is a web application that consists of a quiz, notes, syllabus, old questions, and solutions. It is a comprehensive educational tool for students and educators. The quiz feature allows users to test their knowledge and understanding of various subjects, while the notes section provides a place for users to take and organize notes on specific topics. The syllabus feature gives users access to the curriculum for a particular course or subject, and the old questions and solutions section provides users with past exam questions and their corresponding solutions. This web application is a valuable resource for both students preparing for exams and educators looking to supplement their lesson plans and assignments.",
    role: "FullStack Development",
    stacks: ["Django", "Bootstrap", "Jquery"],
    is_featured: false,
  },
  {
    title: "BlazeCodes",
    slug: "blazecodes",
    type: "website",
    year: 2022,
    href: "https://blazecodes.sarojpradhan.com.np",
    source: null,
    image: "/img/works/blazecodes.png",
    description:
      "Blaze Codes is a blog application that allows users to create and publish blog posts, articles, and tutorials on a wide range of topics related to web development, programming, and the latest technology. It provides a simple and easy-to-use interface for creating and editing blog content, and allows users to customize the appearance of their blogs. With this application, users can share their knowledge and experience with the web development, programming, and technology communities, and engage with other users through comments and discussions. It is an ideal platform for developers, programmers, and tech enthusiasts to share their insights, ideas, and expertise with a global audience.",
    role: "Backend Development",
    stacks: ["Laravel", "React.js", "Inertia.js", "Tailwind"],
    is_featured: false,
  },
  {
    title: "TodoApp",
    slug: "todo-app",
    type: "open source",
    year: 2020,
    href: "https://blaze-todo-app.netlify.app/",
    source: "https://github.com/sarozpradhan64/React-TodoApp",
    image: "/img/works/todoapp.jpg",
    description:
      "It is a static todo listing app which is composed of local storage feature. Also it has filteration option according to complete and uncompleted task",
    role: "Frontend Development",
    stacks: ["React"],
    is_featured: false,
  },
];

export default workData;
